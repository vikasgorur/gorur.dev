...
fun maxProductForAnchor(i: Int, j: Int, grid: Array<Array<Int>>): Int {
    ...
    val down = if (i < GRID_SIZE - 3) {
        grid[i][j] * grid[i+1][j] * grid[i+2][j] * grid[i+3][j]
    } else { 0 }

    // north-west diagonal
    val northWest = if (i >= 3 && j >= 3) {
        grid[i][j] * grid[i-1][j-1] * grid[i-2][j-2] * grid[i-3][j-3]
    } else { 0 }

    // north-east diagonal
    val northEast = if (i >= 3 && j < GRID_SIZE - 3) {
        grid[i][j] * grid[i-1][j+1] * grid[i-2][j+2] * grid[i-3][j+3]
    } else { 0 }

    // south-west diagonal
    val southWest = if (i < GRID_SIZE - 3 && j >= 3) {
        grid[i][j] * grid[i+1][j-1] * grid[i+2][j-2] * grid[i+3][j-3]
    } else { 0 }

    // south-east diagonal
    val southEast = if (i < GRID_SIZE - 3 && j < GRID_SIZE - 3) {
        grid[i][j] * grid[i+1][j+1] * grid[i+2][j+2] * grid[i+3][j+3]
    } else { 0 }
...
easier. I suppose that's the lesson that Python learned as well. I solved this problem in Kotlin and I'm
