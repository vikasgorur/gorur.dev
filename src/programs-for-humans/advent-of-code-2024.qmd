...
The first condition can be understood as the row being _monotonic_: all the deltas have the same sign. We'll make use of the fact that `=` in Clojure can take any number of arguments, and that `apply` allows us to supply the arguments to a function as a sequence.

```clojure
(defn sign [x] (if (zero? x) 1 (/ x (abs x))))
(defn monotonic? [r] (apply = (map sign (deltas r))))
```

<span class="eval">(monotonic? [1 2 3 4 5]) => true</span>
<span class="eval">(monotonic? [5 4 3 2 1]) => true</span>
<span class="eval">(monotonic? [5 4 3 2 7]) => false</span>
