---
title: "Makemore 1: bigram frequencies"
date: 2025-05-26
author: Vikas Gorur
toc: true
toc-location: left
...
Let us define that our _training set_ for language modeling consists of many _documents_. In our example here, each movie name should be considered a document.
...
Given such a model, we can define the _likelihood_ that it produces a particular document $d$. This likelihood is the probability that all possible `(context, next_token)` pairs that are found in the document occur _at the same time_. Thus the likelihood is computed as the product of the model's probabilities for all such pairs, for a given document.

$$
L(d) = p_1 \cdot p_2 \cdot p_3 ... p_k
$$

where each $p_i$ is the probability of one possible token sequence $(t_1 ... t_n, t_{n+1})$.

Let's work this out for the bigram model. Consider the document `SHOLAY`. It consists of the bigrams `{'.S', 'SH', 'OL', 'LA', 'AY', 'Y.'}`. We've added an extra token `.` to signify the beginning and end. Assume now that `M` is a Python function that returns the probability of the next letter given a letter, so for example `M('S', 'H')` is the probability that `H` follows an `S`. The likelihood of `SHOLAY` under this model is therefore:
...
We'll now make a couple of tweaks to make the floating point math better. Instead of working with likelihoods that are very small numbers (because they are the products of many numbers less than $1$), we will instead compute the _negative log-likelihood_ for each document.

$$
\mathrm{NLL}(d) = - (\log p_1 + \log p_2 + \log p_3 \ldots + \log p_k)
$$
...
$$
\mathrm{NLL_avg} = \frac{1}{N} \sum_{i=1}^{N} d_i
$$
...
Let's compute the loss for our bigram model by first defining the negative log-likelihood for a single name:
...
The total loss is the average across all names.
...
In the rest of this series we'll build more and more complicated models and all their training will be driven by the goal of minimizing this loss function.
